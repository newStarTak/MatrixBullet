...
public class OVRGrabber : MonoBehaviour
{
    ...
    protected virtual void GrabBegin()
    {
        ...
        if (closestGrabbable != null)
        {
            ...
            if (gameObject.tag == "HAND_L")
            {
                m_grabbedObj.transform.position = gameObject.transform.position;
                m_grabbedObj.transform.rotation = gameObject.transform.rotation * Quaternion.Euler(-90f, 0f, -90f);

                if (m_grabbedObj.tag == "GUN_L")
                {
                    gameObject.SendMessage("Hand_L_Grap_Gun_L_Toggle", true);
                }
                else if (m_grabbedObj.tag == "GUN_R")
                {
                    gameObject.SendMessage("Hand_L_Grap_Gun_R_Toggle", true);
                }
            }
            else if (gameObject.tag == "HAND_R")
            {
                m_grabbedObj.transform.position = gameObject.transform.position;
                m_grabbedObj.transform.rotation = gameObject.transform.rotation * Quaternion.Euler(90f, -90f, 0f);

                if (m_grabbedObj.tag == "GUN_L")
                {
                    gameObject.SendMessage("Hand_R_Grap_Gun_L_Toggle", true);
                }
                else if (m_grabbedObj.tag == "GUN_R")
                {
                    gameObject.SendMessage("Hand_R_Grap_Gun_R_Toggle", true);
                }
            }
    ...
    protected void GrabbableRelease(Vector3 linearVelocity, Vector3 angularVelocity)
    {
        ...
        if (gameObject.tag == "HAND_L")
        {
            if (m_grabbedObj.tag == "GUN_L")
            {
                gameObject.SendMessage("Hand_L_Grap_Gun_L_Toggle", false);
            }
            else if (m_grabbedObj.tag == "GUN_R")
            {
                gameObject.SendMessage("Hand_L_Grap_Gun_R_Toggle", false);
            }
            else if(m_grabbedObj.tag == "GRENADE_L" || m_grabbedObj.tag == "GRENADE_R")
            {
                m_grabbedObj.SendMessage("GrenadeBoom");
            }
        }
        else if (gameObject.tag == "HAND_R")
        {
            if (m_grabbedObj.tag == "GUN_L")
            {
                gameObject.SendMessage("Hand_R_Grap_Gun_L_Toggle", false);
            }
            else if (m_grabbedObj.tag == "GUN_R")
            {
                gameObject.SendMessage("Hand_R_Grap_Gun_R_Toggle", false);
            }
            else if (m_grabbedObj.tag == "GRENADE_L" || m_grabbedObj.tag == "GRENADE_R")
            {
                m_grabbedObj.SendMessage("GrenadeBoom");
